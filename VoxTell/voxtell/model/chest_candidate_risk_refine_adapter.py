...
class ChestCandidateRiskRefineAdapter(nn.Module):
    """
    v6.5 adapter:
    - keeps v6.4 logit-level candidate proposal + local risk audit
    - adds a bounded serial refine head to softly suppress risky positives
    - exposes refine activations as the suppression tensor for fp-aware training
    """
    ...
    def rectify_logits(
        ...
        candidate_uncertainty = candidate_prob * (1.0 - candidate_prob)
        ...
        refine_bias = self.refine_scale * refine_prob * candidate_prob

        self.last_candidate_mean = float(candidate_prob.detach().mean().cpu())
        self.last_risk_mean = float(risk_prob.detach().mean().cpu())
        self.last_risk_tensor = risk_prob
        self.last_refine_mean = float(refine_prob.detach().mean().cpu())
        self.last_refine_tensor = refine_prob
        return candidate_logits - self.risk_scale * risk_logit - refine_bias
