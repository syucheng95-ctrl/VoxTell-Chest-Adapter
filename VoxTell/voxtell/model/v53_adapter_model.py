...
class VoxTellV53AdapterModel(nn.Module):
    """
    Balanced v5.3 adapter wrapper:
    - keeps the original VoxTell decoder/query path
    - restores category conditioning as a weak bias
    - keeps explicit suppression supervision with a softer effect on features
    """
    ...
    def __init__(
        ...
        adapter_suppression_strength: float,
        adapter_category_scale: float,
        adapter_suppression_scale: float,
        ...
        self.text_guided_adapter = ChestTextGuidedAdapter(
            ...
            suppression_strength=adapter_suppression_strength,
            use_category_bias=use_category_bias,
            category_scale=adapter_category_scale,
            suppression_scale=adapter_suppression_scale,
        )
