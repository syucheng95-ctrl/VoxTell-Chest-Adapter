...
from voxtell.model.chest_candidate_risk_adapter import ChestCandidateRiskAdapter
from voxtell.model.voxtell_model import VoxTellModel


class VoxTellV64AdapterModel(nn.Module):
    """
    v6.4 wrapper:
    - keeps original VoxTell decoder/query path
    - removes feature-space candidate enhancement
    - applies weak-category candidate proposal + logit-level local risk rectification
    """
    ...
    def __init__(
        ...
        self.text_guided_adapter = ChestCandidateRiskAdapter(
    ...
    def forward(
        ...
        for prompt_idx in range(num_prompts):
            ...
            self.last_suppression_mean = self.last_risk_mean
            self.last_suppression_tensor = self.last_risk_tensor
...
# Backward-compatible alias for older imports during transition.
VoxTellV6AdapterModel = VoxTellV64AdapterModel
