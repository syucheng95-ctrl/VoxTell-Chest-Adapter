...
class ChestMaskedRiskRefineAdapterV66b(nn.Module):
    """
    v6.6b adapter:
    - risk acts as bounded candidate-masked FP suppression
    - refine acts as uncertainty-guided local cleanup
    - avoids extra hand-tuned gate parameters beyond the existing scales
    """
    ...
    def __init__(
        ...
        self.last_refine_raw_mean: float | None = None
        self.last_refine_raw_tensor: torch.Tensor | None = None
    ...
    def forward(
        ...
        self.last_refine_raw_mean = None
        self.last_refine_raw_tensor = None
        return feature
    ...
    def rectify_logits(
        ...
        uncertainty = 4.0 * candidate_prob * (1.0 - candidate_prob)
        binary_hint = (candidate_prob > 0.5).float()

        risk_input = torch.cat([candidate_logits, candidate_prob, uncertainty, binary_hint, context], dim=1)
        risk_logit = self.logit_risk_head(risk_input)
        risk_prob = torch.sigmoid(risk_logit)
        effective_risk = risk_prob * candidate_prob.detach()
        ...
        effective_refine = refine_prob * uncertainty.detach()
        ...
        self.last_refine_raw_mean = float(refine_prob.detach().mean().cpu())
        self.last_refine_raw_tensor = refine_prob
